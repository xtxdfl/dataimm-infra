...
export SOLR_LOCATION="/usr/lib/dataimm-infra-solr"
export SOLR_CLIENT_LOCATION="/usr/lib/dataimm-infra-solr-client"
command="$1"
...
function dataimm_migration_helper() {
...
if [[ ! -f "/var/lib/dataimm-infra-solr/data/solr.xml" ]]; then
  cp /infra-solr/conf/solr.xml /var/lib/dataimm-infra-solr/data/
fi
...
case $command in
  ...
  "index-upgrade-tool")
     ...
     ;;
  "dataimm-migration-helper")
     dataimm_migration_helper ${@:2}
     ;;
  *)
     echo "Available commands: (server|client|data-manager|index-upgrade-tool|dataimm-migration-helper|bootstrap-collections)"
     ;;
esac
