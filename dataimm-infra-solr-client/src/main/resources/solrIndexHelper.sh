...
function print_help() {
  cat << EOF
   ...
   upgrade-index command arguments:
     -d, --index-data-dir <DIRECTORY>        Location of the solr cores (e.g.: /opt/dataimm_infra_solr/data)
...
function upgrade_core() {
  local INDEX_DIR=${1:?"usage: <index_base_dir> e.g.: /opt/dataimm_infra_solr/data"}
