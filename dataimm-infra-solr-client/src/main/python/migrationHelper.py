...
DATAIMM_SUDO = "/var/lib/dataimm-agent/dataimm-sudo.sh"

SOLR_SERVICE_NAME = 'DATAIMM_INFRA_SOLR'
...
INFRA_SOLR_CLIENT_BASE_PATH = '/usr/lib/dataimm-infra-solr-client/'
...
def api_accessor(host, username, password, protocol, port):
  def do_request(api_url, request_type, request_body=''):
    try:
      ...
      request.add_header('X-Requested-By', 'dataimm')
...
def create_infra_solr_client_command(options, config, command, appendZnode=False):
  ...
  if kerberos_enabled == 'true':
    ...
    jaasOption=" --jaas-file /etc/dataimm-infra-solr/conf/infra_solr_jaas.conf"
    command+=jaasOption
  ...
  solr_cli_cmd = '{0} {1}{2} /usr/lib/dataimm-infra-solr-client/solrCloudCli.sh --zookeeper-connect-string {3} {4}'\
    .format(DATAIMM_SUDO, set_java_home_, set_infra_solr_cli_opts, zkConnectString, command)
...
def restore(options, accessor, parser, config, original_collection, collection, config_set, index_location, hdfs_path, shards):
  """
  Send restore solr collection custom command request to dataimm-server
  """
  cluster = config.get('dataimm_server', 'cluster')
...
def migrate(options, accessor, parser, config, collection, index_location):
  """
  Send migrate lucene index custom command request to dataimm-server
  """
  cluster = config.get('dataimm_server', 'cluster')
...
def backup(options, accessor, parser, config, collection, index_location):
  """
  Send backup solr collection custom command request to dataimm-server
  """
  cluster = config.get('dataimm_server', 'cluster')
...
def upgrade_solr_instances(options, accessor, parser, config):
  ...
  """
  cluster = config.get('dataimm_server', 'cluster')
  solr_instance_hosts = get_service_components(options, accessor, cluster, "DATAIMM_INFRA_SOLR", "INFRA_SOLR")
  ...
  if options.async_op:
    print("Upgrade request sent to Dataimm server. Check Dataimm UI about the results.")
    sys.exit(0)
  else:
    sys.stdout.write("Start monitoring Dataimm request with id {0} ...".format(request_id))
    sys.stdout.flush()
    cluster = config.get('dataimm_server', 'cluster')
...
def upgrade_solr_clients(options, accessor, parser, config):
  ...
  """
  cluster = config.get('dataimm_server', 'cluster')
  solr_client_hosts = get_service_components(options, accessor, cluster, "DATAIMM_INFRA_SOLR", "INFRA_SOLR_CLIENT")
...
def service_components_command(options, accessor, parser, config, service, component, command, command_str):
  ...
  if options.async_op:
    print("{0} request sent to Dataimm server. Check Dataimm UI about the results.".format(command_str))
    sys.exit(0)
  else:
    sys.stdout.write("Start monitoring Dataimm request with id {0} ...".format(request_id))
    sys.stdout.flush()
    ...
    if 'Requests' in request_response and 'request_status' in request_response['Requests']:
      ...
      if request_status in ['FAILED', 'TIMEDOUT', 'ABORTED', 'COMPLETED', 'SKIPPED_FAILED']:
        if request_status == 'COMPLETED':
          ...
          print("\nRequest (id: {0}) {1}FAILED{2} (checkout Dataimm UI about the failed tasks)\n".format(request_id, colors.FAIL, colors.ENDC))
          sys.exit(1)
        break
      else:
...
def get_request_id(json_response):
  ...
  raise Exception("Cannot access request id from Dataimm response: {0}".format(json_response))
...
def do_backup_request(options, accessor, parser, config, collection, index_location):
  sys.stdout.write("Sending backup collection request ('{0}') to Dataimm to process (backup destination: '{1}')..."
  ...
  if options.async_op:
    print("Backup {0} collection request sent to Dataimm server. Check Dataimm UI about the results.".format(collection))
    sys.exit(0)
  else:
    sys.stdout.write("Start monitoring Dataimm request with id {0} ...".format(request_id))
    sys.stdout.flush()
...
def do_migrate_request(options, accessor, parser, config, collection, index_location):
  sys.stdout.write("Sending migrate collection request ('{0}') to Dataimm to process (migrate folder: '{1}')..."
                   .format(collection, index_location))
  sys.stdout.flush()
  ...
  if options.async_op:
    print("Migrate {0} collection index request sent to Dataimm server. Check Dataimm UI about the results.".format(collection))
    sys.exit(0)
  else:
    sys.stdout.write("Start monitoring Dataimm request with id {0} ...".format(request_id))
    sys.stdout.flush()
...
def do_restore_request(options, accessor, parser, config, original_collection, collection, config_set, index_location, shards, hdfs_path):
  sys.stdout.write("Sending restore collection request ('{0}') to Dataimm to process (backup location: '{1}')..."
                   .format(collection, index_location))
  sys.stdout.flush()
  ...
  if options.async_op:
    print("Restore {0} collection request sent to Dataimm server. Check Dataimm UI about the results.".format(collection))
    sys.exit(0)
  else:
    sys.stdout.write("Start monitoring Dataimm request with id {0} ...".format(request_id))
    sys.stdout.flush()
...
def rolling_restart(options, accessor, parser, config, service_name, component_name, context):
  ...
  print("{0} request sent. (check Dataimm UI about the requests)".format(context))
...
def set_solr_authorization(options, accessor, parser, config, enable_authorization, fix_kerberos_config = False):
  ...
  if kerberos_enabled == 'true':
    ...
    if enable_authorization:
      print("Enable Solr security.json management by Dataimm ... ")
      ...
      print("Disable Solr authorization by uploading a new security.json and turn on security.json management by Dataimm...")
...
if __name__=="__main__":
  ...
  parser.add_option("--async", dest="async_op", action="store_true", default=False, help="async Dataimm operations (backup | restore | migrate)")
  ...
  if options.action is None:
    ...
    if config.has_section('dataimm_server'):
      host = config.get('dataimm_server', 'host')
      port = config.get('dataimm_server', 'port')
      protocol = config.get('dataimm_server', 'protocol')
      username = config.get('dataimm_server', 'username')
      password = config.get('dataimm_server', 'password')
