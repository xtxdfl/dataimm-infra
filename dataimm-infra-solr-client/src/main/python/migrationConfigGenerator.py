...
def generate_dataimm_solr_migration_ini_file(options, accessor, protocol):

  print("Start generating config file: {0} ...".format(options.ini_file))

  config = configparser.RawConfigParser()

  config.add_section('dataimm_server')
  config.set('dataimm_server', 'host', options.host)
  config.set('dataimm_server', 'port', options.port)
  config.set('dataimm_server', 'cluster', options.cluster)
  config.set('dataimm_server', 'protocol', protocol)
  config.set('dataimm_server', 'username', options.username)
  config.set('dataimm_server', 'password', options.password)

  print("Get Dataimm cluster details ...")
  ...
  if solr_urls:
    print("Service detected: " + colors.OKGREEN + "DATAIMM_INFRA_SOLR" + colors.ENDC)
  ...
  infra_solr_kerberos_keytab = infra_solr_env_props['infra_solr_kerberos_keytab'] if 'infra_solr_kerberos_keytab' in infra_solr_env_props else '/etc/security/keytabs/dataimm-infra-solr.service.keytab'
...
if __name__=="__main__":
  try:
    ...
    parser.add_option("-H", "--host", dest="host", type="string", help="hostname for dataimm server")
    parser.add_option("-P", "--port", dest="port", type="int", help="port number for dataimm server")
    ...
    parser.add_option("-s", "--ssl", dest="ssl", action="store_true", help="use if dataimm server using https")
    parser.add_option("-v", "--verbose", dest="verbose", action="store_true", help="use for verbose logging")
    parser.add_option("-u", "--username", dest="username", type="string", help="username for accessing dataimm server")
    parser.add_option("-p", "--password", dest="password", type="string", help="password for accessing dataimm server")
    parser.add_option("-j", "--java-home", dest="java_home", type="string", help="local java_home location")
    parser.add_option("-i", "--ini-file", dest="ini_file", default="dataimm_solr_migration.ini", type="string", help="Filename of the generated ini file for migration (default: dataimm_solr_migration.ini)")
    ...
    if errors:
      ...
      try:
        generate_dataimm_solr_migration_ini_file(options, accessor, protocol)
      except Exception as exc:
