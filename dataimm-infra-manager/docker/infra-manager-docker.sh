...
function build_infra_manager_container() {
  pushd $sdir
  docker build -t dataimm-infra-manager:v1.0 .
  popd
}
...
function start_infra_manager_container() {
 ...
 pushd $sdir/../
 local DATAIMM_INFRA_MANAGER_LOCATION=$(pwd)
 popd
 kill_infra_manager_container
 docker run -d --name infra_manager --hostname infra-manager.apache.org \
   -v $DATAIMM_INFRA_MANAGER_LOCATION/target/package:/root/dataimm-infra-manager -p 61890:61890 -p 5007:5007 \
   dataimm-infra-manager:v1.0
  ip_address=$(docker inspect --format '{{ .NetworkSettings.IPAddress }}' infra_manager)
  echo "Dataimm Infra Manager container started on $ip_address (for Mac OSX route to boot2docker/docker-machine VM address, e.g.: 'sudo route add -net 172.17.0.0/16 192.168.59.103')"
