...
function start_containers() {
  ...
  echo $DATAIMM_INFRA_MANAGER_LOCATION
  cd $DATAIMM_INFRA_MANAGER_LOCATION/docker
...
function setup_env() {
  pushd $sdir/../../
  local DATAIMM_INFRA_LOCATION=$(pwd)
  popd
  local docker_ip=$(get_docker_ip)
  cat << EOF > $sdir/.env
DOCKERIP=$docker_ip
MAVEN_REPOSITORY_LOCATION=$HOME/.m2
DATAIMM_INFRA_LOCATION=$DATAIMM_INFRA_LOCATION
